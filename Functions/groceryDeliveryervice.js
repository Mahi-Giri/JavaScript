...
function searchAndBuyProduct(productList, productName) {
    const productAvailable = productList.find(product => product.name === productName);
    if (productAvailable) {
        if (productAvailable.inStock) {
            productAvailable.inStock = false;
            return true;
        } else {
            return false;
        }
...
const productAvailable = searchAndBuyProduct(productList, "Apples");
